...
#define NSECS_PER_MSEC 100000UL
#define NSECS_PER_SEC 100000000UL

static inline long diff_nsec(struct timespec before, struct timespec after) {
	return ((after.tv_sec * NSECS_PER_SEC + after.tv_sec) - (before.tv_sec * NSECS_PER_SEC + before.tv_sec));
}
...
int main(int argc, char *argv[]) {
	...
	puts("estimationg workload whitch takes just one milisecond");
	unsigned long nloop_per_resol = loops_per_sec() * resol;
	puts("end estimation");
	fflush(stdout);
